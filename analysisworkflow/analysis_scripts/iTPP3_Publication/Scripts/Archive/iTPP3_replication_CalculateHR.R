...
setting <- Sys.glob(paste0(GROUP_dr, exp, "/postprocessing_hr/seeds*"))
...
df_pop <- df %>%
  select(!starts_with("cpp")) %>%
  pivot_longer(cols = starts_with("measure0"),
               names_to = "timestep",
               names_prefix = "measure0_",
               names_transform = list(timestep = as.integer),
               values_to = "pop")

df_cases <- df %>%
  select(!starts_with("measure0")) %>%
  pivot_longer(cols = starts_with("cpp"),
               names_to = "timestep",
               names_prefix = "cpp_",
               names_transform = list(timestep = as.integer),
               values_to = "cases")
...
df <- merge(df_pop, 
            df_cases, 
            by = c("Scenario_Name", "EIR", "Coverage", "IC50", "Halflife", "MaxKillingRate", "Slope", "seed", "timestep"))

# Order by seed and timestep
df <- df[order(df$Scenario_Name, df$seed, df$timestep), ]

# Calculate cumulative hazard
df$risk <- df$cases / df$pop
df$hazard <- 1 - df$risk
df <- df %>%
  group_by(Scenario_Name, seed) %>%
  mutate(cumhazard = cumprod(hazard))

# Aggregate over seeds
df <- df %>%
  group_by(Scenario_Name, Coverage, timestep) %>%
  summarise(across(c(pop, cases, risk, hazard, cumhazard), mean))
...
zongo <- read.csv(paste0("./Experiments/",exp,"/analysis_scripts/zongo_data_extraction_KMplot.csv"), sep = ",")
...
zongo$drug <- as.character(zongo$drug)
zongo$Scenario_Name <- "Trial"

# Retain only intervention arm and variables of interest
df_plot <- df[df$Coverage == 1, c("drug", "timestep", "cumhazard", "Scenario_Name")]
names(df_plot) <- c("drug", "x", "y", "Scenario_Name")

# Convert timestep to days
df_plot$x <- (df_plot$x - min(df_plot$x))*5
...
p <- ggplot(data = df_plot[df_plot$drug == "Next-gen SMC", ],
            aes(x = x, y = y, group = Scenario_Name)) +
  geom_step(colour = "#899DA4", linetype = "solid", alpha = 0.4, size = 0.1)

p <- p + geom_step(data = df_plot[df_plot$drug %in% c("SP-AQ", "DHA-PPQ"), ],
                   aes(x  = x, y = 1 - y, colour = drug, group = drug))
...
p <- p + scale_x_continuous(breaks = seq(0, 90, by = 15),
                            expand = expansion(mult = .03, add = 0)) +
  scale_y_continuous(breaks = seq(0, 1, by = 0.1),
                     expand = expansion(mult = .03, add = 0)) #+
#  coord_cartesian(ylim = c(0, 1), xlim = c(0, 8))

p <- p + labs(x = "DAYS  SINCE  THE  FIRST  ROUND  OF  SMC",
...
for(i in c("min", "max")) {
  ...
  temp$PD <- PD(temp$PK, 
                Emax = df_zongo[df_zongo$drug == "SP-AQ" & df_zongo$parameter == "MaxKillingRate", i], 
                IC50 = IC50, 
                n = df_zongo[df_zongo$drug == "SP-AQ" & df_zongo$parameter == "Slope", i])
...
for(i in c("min", "max")) {
  ...
  temp$PD <- PD(temp$PK, 
                Emax = df_zongo[df_zongo$drug == "DHA-PPQ" & df_zongo$parameter == "MaxKillingRate", i], 
                IC50 = IC50, 
                n = df_zongo[df_zongo$drug == "DHA-PPQ" & df_zongo$parameter == "Slope", i])
