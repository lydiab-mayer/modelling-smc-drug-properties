...
out1[1, ] <- c("Scenario", "Target", "Coverage", "Halflife", "Efficacy")

out2 <- matrix(nrow = 1, ncol = 5)
out2[1, ] <- c("Scenario", "Target", "Coverage", "Halflife", "Efficacy")
...
for(exp in exp_list) {
  ...
  for(i in 1:length(setting_id)){
    ...
    # Set up tables to store results
    out_int1 <- matrix(nrow = 1 + length(targets), ncol = 5)
    out_int1[1, ] <- c("Scenario", "Target", "Coverage", "Halflife", "Efficacy")
    out_int1[2:(1 + length(targets)), 1] <- setting_id[i]
    out_int1[2:(1 + length(targets)), 2] <- targets
    
    out_int2 <- matrix(nrow = 1 + length(targets), ncol = 5)
    out_int2[1, ] <- c("Scenario", "Target", "Coverage", "Halflife", "Efficacy")
    out_int2[2:(1 + length(targets)), 1] <- setting_id[i]
    out_int2[2:(1 + length(targets)), 2] <- targets
    
    # Read in optimisation result
    df <- readRDS(setting[i])$scenario
    df$CoverageAllRounds <- df$Coverage1*df$Coverage2^num_rounds[exp]
    
    df1 <- df[df$Halflife < 28,]
    df2 <- df[df$Halflife >= 28, ]
    
    # Extract minimum criteria for selected targets
    for (j in 1:length(targets)){
      out_int1[j + 1, 3] <- min(df1[df1$mean >= targets[j], "CoverageAllRounds"])
      out_int1[j + 1, 4] <- min(df1[df1$mean >= targets[j], "Halflife"])
      out_int1[j + 1, 5] <- min(df1[df1$mean >= targets[j], "Efficacy"])
      
      out_int2[j + 1, 3] <- min(df2[df2$mean >= targets[j], "CoverageAllRounds"])
      out_int2[j + 1, 4] <- min(df2[df2$mean >= targets[j], "Halflife"])
      out_int2[j + 1, 5] <- min(df2[df2$mean >= targets[j], "Efficacy"])
    }
    
    # Store outputs
    out1 <- rbind(out1, out_int1[2:(1 + length(targets)), ])
    out2 <- rbind(out2, out_int2[2:(1 + length(targets)), ])
  }

  # Save resulting matrix against future use
  saveRDS(out1, paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/", exp, "_tradeoffs_cutoff_criteria_less28days.rds"))
  saveRDS(out2, paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/", exp, "_tradeoffs_cutoff_criteria_greater28days.rds"))
...
# ----------------------------------------------------------
# Generate and format database of results across all experiments - < 28 days halflife
# ----------------------------------------------------------

# Merge results across experiments

# !!! Note that this must be updated manually should the list of experiments be changed !!!
df3 <- readRDS(paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/iTPP3_tradeoffs_3rounds_tradeoffs_cutoff_criteria_less28days.rds"))
df4 <- readRDS(paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/iTPP3_tradeoffs_4rounds_tradeoffs_cutoff_criteria_less28days.rds"))
df5 <- readRDS(paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/iTPP3_tradeoffs_5rounds_tradeoffs_cutoff_criteria_less28days.rds"))
...
saveRDS(df, paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/_tradeoffs_cutoff_criteria_less28days.rds"))
...
# !!! Note that this must be updated manually should the list of experiments be changed !!!
df3 <- readRDS(paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/iTPP3_tradeoffs_3rounds_tradeoffs_cutoff_criteria_greater28days.rds"))
df4 <- readRDS(paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/iTPP3_tradeoffs_4rounds_tradeoffs_cutoff_criteria_greater28days.rds"))
df5 <- readRDS(paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/iTPP3_tradeoffs_5rounds_tradeoffs_cutoff_criteria_greater28days.rds"))
...
saveRDS(df, paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/_tradeoffs_cutoff_criteria_greater28days.rds"))
...
# Calculate max across all scenarios
df_plot <- df_plot %>%
  group_by(Endpoint, Target, annual_prev) %>%
...
p <- p + labs(title = "DURATION OF PROTECTION <28 DAYS", y = "", x = expression(paste(bold("BASELINE ANNUAL "), bolditalic("Pf"), bold("PR"["2-10"])))) +
...
ggsave(filename = paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/iTPP3_tradeoffs_cutoff_criteria_efficacy_FIG432.jpg"),
       plot = last_plot(),
       width = 9,
       height = 4,
       dpi = 200)
