...
for(exp in exp_list) {
  ...
  df_plot$Seasonality <- ifelse(df_plot$Seasonality == "sharpseasonal", "FIVE-MONTH SEASON", "SIX-MONTH SEASON")
  df_plot$Seasonality <- factor(df_plot$Seasonality, levels = c("FIVE-MONTH SEASON", "SIX-MONTH SEASON"))
  ...
  ggsave(filename = paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/iTPP3_", plot_name, ".jpg"),
         plot = last_plot(),
         width = 9.1,
         height = 7.5,
         dpi = 200)
