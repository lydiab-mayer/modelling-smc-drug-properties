...
setting <- Sys.glob(paste0(GROUP_dr, exp, "/gp_under5_2035/GP_grid_optimization/", predictors[1], "/*"))
...
for (pred in predictors) {
  ...
  setting <- Sys.glob(paste0(GROUP_dr, exp, "/gp_under5_2035/GP_grid_optimization/", pred, "/*"))
  setting_id <- sub(".rds", "", sub("opt_", "", basename(setting)))
  
  for (j in setting_id[index]) {
    # Load GP model
    load(paste0(GROUP_dr, exp, "/gp_under5_2035/trained/", pred, "/seeds_", j, "_cv.RData"))
...
saveRDS(data, "./data_and_visualisation/Appendix_Figure210/data_figA210_panelA.rds")
