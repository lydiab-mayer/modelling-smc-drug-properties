...
p <- ggplot(data[["MaxKillingRate"]], aes(x = MaxKillingRate, y = mean, ymin = cl, ymax = cu, linetype = pred)) +
#  geom_ribbon(alpha = 0.3, fill = col) +
#  geom_point(colour = col) +
  geom_line(colour = col)
...
p <- ggplot(data[["slope"]], aes(x = Slope, y = mean, ymin = cl, ymax = cu, linetype = pred)) +
#  geom_ribbon(alpha = 0.3, fill = col) +
#  geom_point(colour = col) +
  geom_line(colour = col)
...
p_out <- (p_coverage1 | p_coverage2) / (p_halflife | p_maxkilling) + plot_annotation(title = "A") & theme(plot.title = element_text(family = "serif", face = "bold"))
...
ggsave(filename = paste0("./data_and_visualisation/Manuscript_Figure2/fig2_panelA.pdf"),
       plot = p_out,
       width = 4.5,
       height = 4.35,
       dpi = 400)