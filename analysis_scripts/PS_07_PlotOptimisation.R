...
df <- readRDS(setting[1])$scenarios
head(df)
...
p <- p + theme(panel.border = element_blank(), 
               ...
               text = element_text(colour = "#323f4f", family = "Courier"),
...
p <- p + scale_y_continuous(labels = scales::percent_format(accurary = 1L)) +
 scale_fill_continuous(high = "#1c649e", 
                       low = "#ffffff", 
                       breaks = seq(30, 85, 10), 
                       limits = c(30, 85),
                       labels = paste0(seq(30, 85, 10), "%")) +
  scale_x_continuous(breaks = seq(10, 60, 5))

p <- p + labs(fill = "REDUCTION",
              x = "DURATION OF PROTECTION HALFLIFE (DAYS)",
              y = "INITIAL EFFICACY")
