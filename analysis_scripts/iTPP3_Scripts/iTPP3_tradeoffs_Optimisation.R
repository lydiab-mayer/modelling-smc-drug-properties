...
p <- p + geom_rect(data = df_plot[1, ], 
  ...
  geom_label(label = "DHA+PPQ", x = 42, y = .97, fill = "white", label.size = NA, family = "Courier", fontface = "bold")
...
p1 <- p1 + theme(panel.border = element_blank(), 
               panel.background = element_blank(),
               panel.grid = element_blank(),
               text = element_text(family = "Courier", size = 8),
               strip.background = element_blank(),
               axis.line = element_blank(),
               axis.ticks = element_blank(),
               axis.text.x = element_text(margin = margin(t = 0)),
...
p / (p1 | p2 | p3 | p4) + plot_layout(heights = unit(c(7, 1), c('cm', 'null')))

ggsave(filename = paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/iTPP3_tradeoffs_", exp, "_", pred, "_FIG333.jpg"),
       plot = last_plot(),
       width = 9,
       height = 5,
       dpi = 200)
...
ggsave(filename = paste0("./analysisworkflow/analysis_scripts/iTPP3_Figures/iTPP3_tradeoffs_", exp, "_", pred, "_FIG5332.jpg"),
       plot = last_plot(),
       width = 9,
       height = 5.6,
       dpi = 200)