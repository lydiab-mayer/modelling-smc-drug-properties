...
for(exp in exp_list) {
  ...
  df <- df[df$parameter %in% c("Halflife", "Efficacy"), ]
  df <- df[df$Access == 0.24, ]
  df <- df[df$Outcome == "prev", ]
  df <- df[df$Agegroup == 5, ]
  ...
  df$parameter <- factor(df$parameter, levels = c("Halflife", "Efficacy"))
  ...
  p <- p + facet_wrap(.~ Seasonality, scales = "free_x")
  ...
  p <- p + labs(x = expression(paste(bold("BASELINE ANNUAL "), bolditalic("Pf"), bold("PR"["2-10"]))),
                y = expression(paste(bold("MEDIAN "), bolditalic("Pf"), bold("PR"["3-59"]), bold(" REDUCTION (%)"))),
                fill = "")
    
  p
    
  ggsave(filename= paste0("./Experiments/", exp, "/Outputs/PS_06_", plot_name, "FIGCLINICALTRANSLATION_SENS.jpg"),
         plot = last_plot(),
         width = 9.1,
         height = 5,
         dpi = 200)
