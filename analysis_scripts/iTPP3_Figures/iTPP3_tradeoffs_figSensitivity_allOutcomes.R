...
for(exp in exp_list) {
  ...
  df_impact <- df_impact[, c("inc", "sev", "mor", "scenario")] %>%
    separate(col = scenario, 
  ...
  df_impact <- df_impact %>%
    pivot_longer(cols = c(inc, sev, mor), names_to = "Outcome", values_to = "Median_Reduction")
  ...
  plot_name <- paste0(exp, "_sensitivity_results")
  ...
  y_axis <- c("MEDIAN INCIDENCE REDUCTION", "MEDIAN SEVERE DISEASE REDUCTION", "MEDIAN MORTALITY REDUCTION")
  names(y_axis) <- pred_list
    
  
  # ----------------------------------------------------------
  # Generate plot - figure 1.3.2
  # ----------------------------------------------------------
  ...
  p <- p + labs(x = expression(paste(bolditalic("Pf"), bold("PR"["2-10"]))),
                y = "MEDIAN REDUCTION (%)",
                fill = "")
  ...
  ggsave(filename= paste0("./Experiments/", exp, "/Outputs/PS_06_", plot_name, "FIG132.jpg"),
         plot = last_plot(),
         width = 9.1,
         height = 5,
         dpi = 200)
  ...
  write.csv(tab[, c(1, 2, 5)], file = paste0("./Experiments/", exp, "/Outputs/DF_06_", plot_name, "TAB132.csv"))
    
    
  # ----------------------------------------------------------
  # Generate plot - figure 3.1.1
  # ----------------------------------------------------------
  ...
  p <- p + facet_wrap(. ~ Agegroup + Outcome, scales = "free_x", ncol = 3)
  ...
  ggsave(filename= paste0("./Experiments/", exp, "/Outputs/PS_06_", plot_name, "FIG311.jpg"),
         plot = last_plot(),
         width = 9.1,
         height = 8,
         dpi = 200)
  ...
  ggsave(filename= paste0("./Experiments/", exp, "/Outputs/PS_06_", plot_name, "FIG431.jpg"),
         plot = last_plot(),
         width = 9.1,
         height = 5,
         dpi = 200)
