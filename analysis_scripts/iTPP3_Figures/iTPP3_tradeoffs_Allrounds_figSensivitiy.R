...
for (pred in pred_list) {
  ...
  plot_name <- paste0(exp_list[1], "_", pred, "_sensitivity_results")
  ...
  # ----------------------------------------------------------
  # Generate plot - figure A.2.2
  # ----------------------------------------------------------
  
  p <- ggplot(df[df$Access == "HIGH ACCESS" & (df$Agegroup == "CHILDREN 3 TO 59 MONTHS" & df$EIR == 32), ], 
  ...
  ggsave(filename= paste0("./Experiments/", exp_list[1], "/Outputs/PS_06_", plot_name, "FIGA22.jpg"),
         plot = last_plot(),
         width = 9.1,
         height = 2.5,
         dpi = 200)
