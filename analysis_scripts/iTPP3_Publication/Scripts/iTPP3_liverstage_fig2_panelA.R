...
pred <- pred_list
  ...
  p <- ggplot(df, aes(x = Efficacy, y = mean, ymin = cl, ymax = cu)) +
    geom_ribbon(alpha = 0.3, fill = col) +
    geom_point(colour = col)
  ...
  p_out <- (p_coverage1 | p_coverage2) / (p_halflife | p_efficacy) + plot_annotation(title = "A") & theme(plot.title = element_text(family = "Times New Roman", face = "bold"))
