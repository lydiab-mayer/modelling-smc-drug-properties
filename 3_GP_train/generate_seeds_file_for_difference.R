...
seeds_smc_smc <- data.frame(read.table(paste("/scicore/home/penny/GROUP/smc_lai/E3_SMCSMC/postprocessing_5/seeds_E3_SMCSMC_",scenario[,"Seasonality"],"_",scenario[,"IntAge"],"_",scenario[,"LAI_dec"],"_",scenario[,"Access"] ,".txt", sep = ""), header = T, as.is = TRUE, stringsAsFactors = FALSE))

seeds_smc_smc <- seeds_smc_smc[order(seeds_smc_smc$Scenario_Name, seeds_smc_smc$seed),]
seeds_smc_lai <- data.frame(read.table(paste("/scicore/home/penny/GROUP/smc_lai/E4_SMCLAI/postprocessing_5/seeds_E4_SMCLAI_",scenario[,"Seasonality"],"_",scenario[,"IntAge"],"_",scenario[,"LAI_dec"],"_",scenario[,"Access"] ,".txt", sep = ""), header = T, as.is = TRUE, stringsAsFactors = FALSE))
...
seeds_comparison_file <- paste0("/scicore/home/penny/GROUP/smc_lai/E3_E4_comparison/postprocessing_5/seeds_E3_E4_comparison_",scenario[,"Seasonality"],"_",scenario[,"IntAge"],"_",scenario[,"LAI_dec"],"_",scenario[,"Access"] ,".txt")

file.remove(seeds_comparison_file)
write.table(seeds_comparison, file = seeds_comparison_file,
			    sep = "\t", col.names = T, row.names = F)

agg_comparison_file <- paste0("/scicore/home/smith/burlyd00/smc_lai/E3_E4_comparison/postprocessing_5/agg_E3_E4_comparison_",scenario[,"Seasonality"],"_",scenario[,"IntAge"],"_",scenario[,"LAI_dec"],"_",scenario[,"Access"] ,".txt")
