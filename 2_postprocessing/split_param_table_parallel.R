...
names(list_tab_splits_test) = paste0(names(list_tab_splits), "_", seq(1:length(list_tab_splits_test)))

# Write each split to a file named according to the corresponding EIR and seasonality
for (i in 1:length(list_tab_splits_test)) {
  final_table_dest = paste(destination_folder, experiment_name, "_",
                           names(list_tab_splits_test)[i], ".txt", sep="")
  write.table(list_tab_splits_test[[i]], final_table_dest, sep="\t",
